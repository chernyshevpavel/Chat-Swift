    ...
    @objc private func sendMessage() {
        ...
        self.dismiss(animated: true) {
            FirestoreService.shared.createWaitingChat(message: message, receiver: self.user) { (result) in
                ...
                case .success:
                    UIApplication.getTopViewController()?.showAlert(with: "Successful!".lowercased(), and: String(format: "Your message for %@ was send".lowercased(), self.user.username))
                case .failure(let error):
                    UIApplication.getTopViewController()?.showAlert(with: "Error".localizedCapitalized, and: error.localizedDescription)
                }
            }
